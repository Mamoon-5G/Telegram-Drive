...
const SANDBOXED_MESSAGE_ORIGIN = 'null';
...
export function DesktopAdBanner({ suppressed = false, onSupport }: DesktopAdBannerProps) {
  ...
  useEffect(() => {
    ...
    const receiveStatus = (event: MessageEvent<unknown>) => {
      if (event.origin !== SANDBOXED_MESSAGE_ORIGIN || event.source !== iframeRef.current?.contentWindow) return;
  ...
  useEffect(() => {
    if (!eligible || !visible || exiting || isHovering || loadStatus === 'loading') return;
    if (countdown <= 0) {
      dismiss();
      return;
    }
  ...
  }, [countdown, dismiss, eligible, exiting, isHovering, loadStatus, visible]);
